...
% RUN both local and cooperative observers for multiple runs changing Q, R,
% c and noise values. The topology and reference are fixed. 

% Define parameter ranges to test
rng(42)
Q_values = {eye(2), 2*eye(2), 100*eye(2)};
R_values = [0.1, 1, 100];
noise_levels = [0, 200, 2500, 10000];
c_weights = [5, 10, 50];
T_sim = 10;

topology_type = 'star'; % Possible values: chain, star, tree, complete
reference = 'sin';      % Possible values: const, ramp, sin
noise_sample_time = 1;
threshold = 2e-4;
...
for iq = 1:length(Q_values)
  Q = Q_values{iq};
    for ir = 1:length(R_values)
        R = R_values(ir);
        for in = 1:length(noise_levels)
            noise_level = noise_levels(in);
            for ic = 1:length(c_weights)
                fprintf('\nExperiment %d: Q=%.1f*I, R=%.2f, noise=%.3f, c=%d\n', ...
                    exp_num, trace(Q)/2, R, noise_level, c_weights(ic));
                exp_num = exp_num + 1;
